...
export default function EditReviewModal(props: Props) {
  ...
  return (
    <>{editModalIsOpen &&
      <div onClick={() => setEditModalIsOpen(false)} className="review-modal__tint"></div>
    }
      <div className="review-modal">
        <div className="review-modal__container">
          <h2 className="review-modal__title">{`Edit your review of ${gymName}`}</h2>
          <form onSubmit={handleSubmit} className="review-form">
            ...
            <textarea onChange={handleTextChange} className="review-form__description" name="review-form-description" id="" cols={30} rows={10} value={review.description} placeholder="Share details of your experience"></textarea>
            <div>
              <button className="review-form__submit-btn" type="submit">Submit</button>
              <button onClick={handleDeleteReview} className="review-form__delete-btn" type="button">Delete Review</button>
            </div>
          </form>
        </div>
      </div>
